...
struct Quantizer <:AbstractTransformer
    ...
    dsize::UInt8
    factor::UInt8
    Quantizer(bsize, dsize, factor) = new(bsize, dsize, factor)
    Quantizer(bsize) = initialize_quantizer(bsize)
end

function initialize_quantizer(bsize::UInt8)
    dsize = 0x08 - bsize
    factor = 0x02 ^ dsize
    return Quantizer(bsize, dsize, factor)
end
...
#     basis = @. floor(UInt8, data / quantizer.factor)
#     deviation = data - (quantizer.factor * basis)
#     basis = reduce(vcat, digits.(Bool, basis, base=2, pad=quantizer.bsize)) |> BitVector
#     deviation = reduce(vcat, digits.(Bool, deviation, base=2, pad=quantizer.dsize)) |> BitVector
...
# """
# function invtransform(quantizer::Quantizer, basis::BitVector, deviation::BitVector)
#     b = [bits2uint(basis[i:i-1+quantizer.bsize]) for i ∈ 1:quantizer.bsize:length(basis)]
#     d = [bits2uint(deviation[i:i-1+quantizer.dsize]) for i ∈ 1:quantizer.dsize:length(deviation)]
#     return 0x02 ^ quantizer.dsize * b + d
# end
